...
Token presence is read via the **synchronous** `isAuthenticated` getter on the storage seam — `GoRoute.redirect` cannot await. That means the seam's async `token` getter must be warmed once during app init so the in-memory cache is populated before the first navigation (see `fl-data-layer`).
