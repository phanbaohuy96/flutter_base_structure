...
- [ ] If present, the coordinator is an extension on `BuildContext` using `PushBehavior` — added only for compound modules or non-trivial entry logic (see `fl-route-config` §Coordinator).
